...
void Main()
{
	...
	// put in all the mins from the inbetween quickly noted lost time from the lotish
...
public class Lotish
{
	...
	public static string HourMinsDifferenceString
	{
		get
		{
			Console.Write(Lotish.TimeDifferenceTotal + " - " + Lotish.TotalTimeLost + "... "); 
			int totalMins = Lotish.TimeDifferenceTotal - Lotish.TotalTimeLost;//this is calculating
	...
	public static string TimeAdd(int hour1, int min1, string am, int hour2, int min2, string pm) {
		...
		Lotish.TimeDifferenceTotal = (hourDiff*60) + minDiff; 
	...
	public static string StudyAdd(int[] hour, int[] min, string plusOrMinus) {
		...
		Lotish.TotalTimeLost = totalTimeInMins; //setting this class's property
		
		//right here I am just testing to make sure that this.Class's property has been set
		//Console.WriteLine("test1... TotalTimeLost for this.Class = " + Lotish.TotalTimeLost); 
