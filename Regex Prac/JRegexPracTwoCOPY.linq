...
{
	string sample = "1) 12-2227777 2) 225-25-4659 3) 77-8577777 4) 456-147852 5) 645-52-7894";
	...
	Console.WriteLine ("\n... Now using .Match and hash codes ...\n");
	...
	//well hello world
	Console.WriteLine ("\n\n\t\tnow using Group test");
	MatchCollection groupMC = Regex.Matches(sample, pattern2); 
...
//now I am prac looping quatifiers from the sub. sec. what backreferecnes match from the 
//main sec of MSDN Backreferences 
public void JLoopingQuantifiersPrac()
{
	//              0            13      19                    39?
	string input = " a 0) aab 1) aababb 2) aababbaababbabbbabbbb  3) abb  4) abbabbbb  5)  aababbabbbabbbbabbbabbbb ";
	string input2 = "aababb";
	string input3 =  "aababbabbbabbbbabbbabbbb";
	string pattern = @"\b(?'1'a)(?'a'\1b)*\b";//what this'll do is create a new group 'a'
	string pattern2 = @"\b(?<1>a)(?<1>\1b)*\b";//rem the * is what makes it a looping quatifier
	string pattern3 = @"(?'1'a)(?'1'\1b)*";
	string pattern4 = @"\b(?'1'a)(?'1'\1b)*\b";
	//this one finally works!!!!!! Took me 2hours to figure out
	MatchCollection mc = Regex.Matches(input3, pattern3);
	MatchCollection mc2 = Regex.Matches(input, pattern3);
	foreach(Match m in mc2)
	{
		Console.WriteLine("\n\n\t~Match value: "+m.Value+", at index: "+m.Index);
		Console.WriteLine("group 1 and 2: "+m.Groups[0]+", "+m.Groups[1]+", "+m.Groups[2]+"...");
		foreach(Group g in m.Groups) 
			Console.WriteLine("      Group value: "+g.Value+", at index: "+g.Index);
	}
}
