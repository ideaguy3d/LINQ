<Query Kind="Statements">
  <Connection>
    <ID>61e9f10d-bc20-4f15-8e6f-7f56fff94e67</ID>
    <Persist>true</Persist>
    <Driver Assembly="IQDriver" PublicKeyToken="5b59726538a49684">IQDriver.IQDriver</Driver>
    <Provider>Devart.Data.MySql</Provider>
    <CustomCxString></CustomCxString>
    <Server>127.0.0.1</Server>
    <Database>edhub_db1</Database>
    <UserName>edhub-admin</UserName>
    <Password></Password>
    <NoPluralization>true</NoPluralization>
    <NoCapitalization>true</NoCapitalization>
    <DisplayName>edhub_db1_v1</DisplayName>
    <EncryptCustomCxString>true</EncryptCustomCxString>
    <DriverData>
      <StripUnderscores>false</StripUnderscores>
      <QuietenAllCaps>false</QuietenAllCaps>
    </DriverData>
  </Connection>
</Query>

/*
	Julius Alvarado 
	Remote Data prac
	5-8-2018
*/

var ds = majide_fba_sales_v1; // the majide FBA_ORDER_BY_DATE_ALL Amazon MWS operation table

var shippedProducts = ds.Where(d => (Boolean)d.order_status.Contains("Shipped") == true);

int count = 0;

foreach (var element in shippedProducts)
{
	++count;
}

Console.WriteLine("The size of Shipped Products = "+count);

foreach (var element in shippedProducts)
{
	Console.WriteLine(element); 
}